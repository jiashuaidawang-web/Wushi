...
VALUES
    ...
    ('PATTERN_CONSENSUS_SCORE', '一致强度', 'DIVERGENCE_CONSENSUS', 'POSITIVE', '板块内龙头、中军、后排同步修复形成的一致强度。', 'plate_intraday_emotion_snapshot', 'NUMBER'),
    ...
    ('PATTERN_REAR_FEEDBACK', '后排反馈', 'DIVERGENCE_CONSENSUS', 'POSITIVE', '后排是否修复、是否继续补涨，验证主线分歧是否被承接。', 'stock_plate_daily_snapshot', 'NUMBER'),
...
VALUES
    ...
    ('COMBO_DIVERGENCE_TO_CONSENSUS', '分歧转一致候选组合', 'DIVERGENCE_CONSENSUS', 'v0.1.0',
     '["PATTERN_DIVERGENCE_SCORE","PATTERN_REFILL_QUALITY","PATTERN_CONSENSUS_SCORE","PATTERN_REAR_FEEDBACK","PATTERN_BROKEN_LIMIT_RISK"]',
     'PATTERN_DIVERGENCE_SCORE >= 0.60 AND PATTERN_REFILL_QUALITY >= 0.60 AND PATTERN_CONSENSUS_SCORE >= 0.65 AND PATTERN_REAR_FEEDBACK >= 0.55 AND PATTERN_BROKEN_LIMIT_RISK <= 0.35',
     '市场经历充分分歧后，龙头回封质量提升，后排开始修复，具备分歧转一致候选特征。'),
